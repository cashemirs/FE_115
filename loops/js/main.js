function check_palindrome() {
    const user_value = document.getElementById("user_value_palindrome").value;
    if (user_value.length != 5) {
        console.log("it is not 5 numeric value")

        return;
    }
    const first_three = user_value.slice(0, 3);
    let last_three = user_value.slice(-3);
    let last_three_reversed = "";

    while (last_three > 0) {
        let temp = last_three % 10;
...
function get_discount() {
    ...
    for (let i = 1; i <= 10; i++) {
        let user_input = parseInt(prompt("Введіть число " + i + ":"));
...
function get_week_days() {
    ...
    while (true) {
        ...
        if (user_response !== null) {
            console.log(typeof user_response + i);
            i++;
        } else {
            return;
        };
...
function akhinator() {//to be honest i needed Ai help to resolve the task, but i got the alhorithm 
    let min = 0;
    let max = 100;
    
    while (true) {
        let guess = Math.floor((min + max) / 2);
        let userResponse = prompt(`Ваше число > ${guess}, < ${guess} або == ${guess}? Введіть один з символів (>, <, =)`);
    
        if (userResponse === '=') {
            alert(`Гра завершена. Ваше число - ${guess}`);
            break;
